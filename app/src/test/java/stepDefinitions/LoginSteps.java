...
public class LoginSteps {
    ...
    public void loginIntoThePage() {
        ...
        System.out.println("The is logging into the page...");
    }

    @Then("The user session is opened")
    public void sessionOpened() {
        Assert.assertEquals(664426, response.jsonPath().getInt("id"));
        Assert.assertEquals("rodrigo.bernal", response.jsonPath().
                getString("username"));
        ...
        Assert.assertEquals("rodrigo.bernal@jala.university", response.jsonPath()
                .getString("email"));
        Assert.assertEquals("2022-12-21T15:02:16.417500Z", response.jsonPath()
                .getString("date_joined"));
        System.out.println("The user session is opened");
    }
    ...
    public void theUserDoesNotHaveAValidAccount() {
        System.out.println("The user does not have a valid account");
    }
    ...
    public void theUserEntersUsernameAndPassword(String username, String password) {
        ...
        System.out.println("The user is trying to login into the page");
    }

    @Then("The user session is not opened")
    public void theUserSessionIsNotOpened() {
        Assert.assertEquals("[Incorrect username or password.]",
                response.jsonPath().getString("non_field_errors"));
        System.out.println("The user cannot login into the page");
    }

    @And("A Status Code Bad Request is displayed")
    public void aStatusCodeUnahutorizationIsDisplayed() {
